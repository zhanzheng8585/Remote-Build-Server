...
namespace MessagePassingComm
{
    ...
    public class Receiver : IMessagePassingComm
    {
        ...
        public CommMessage getMessage()
        {
            ...
            if (msg.type == CommMessage.MessageType.connect && msg.arguments.Capacity == 0)
            {
        ...
        public bool openFileForWrite(string name, string filefrom, string fileto)
        {
            try
            {
                //string filename = ServiceEnvironment.fileStorage + "/task" + fileto;
                string filename = fileto;
                string writePath = Path.Combine(filename, name);
    ...
    public class Sender
    {
        ...
        public bool postFile(string fileName, string filefrom, string fileto)
        {
            FileStream fs = null;
            long bytesRemaining;
            //string filename = "task" + filefrom + "\\" + fileName;
            try
            {
                //string path = Path.Combine(ClientEnvironment.fileStorage, fileName);
                string path = Path.Combine(filefrom, fileName);
                fs = File.OpenRead(path);
                bytesRemaining = fs.Length;
                channel.openFileForWrite(fileName, filefrom, fileto);
                while (true)
                {
    ...
    class TestPCommService
    {
        ...
        public static bool testComm()
        {
            ...
            if (ClientEnvironment.verbose){ crcvMsg.show(); }
            crcvMsg = comm.getMessage();
            if (ClientEnvironment.verbose){ crcvMsg.show(); }
            if (!compareMsgs(csndMsg, crcvMsg)){ test = false; }
            ...
            foreach (string name in names){
                ...
                bool transferSuccess = comm.postFile(name, "1", "1");
                TestUtilities.checkResult(transferSuccess, "transfer");
            }
            foreach (string name in names){
                if (!compareFileBytes(name)){testFileTransfer = false; break;
                }
            }
            ...
            comm.postMessage(csndMsg);
            if (ClientEnvironment.verbose){ csndMsg.show(); }
